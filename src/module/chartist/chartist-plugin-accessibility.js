        ...
        return function ctAccessibility (chart) {
          ...
          // Create table body with caption
            ...
            // Add column headers inclusing the series column header for the row headers
              ...
              // Should support if both is array
              const isNested = Array.isArray(chart.data.series) && Array.isArray(chart.data.series[0])
              if (isNested) {
                ...
                const seriesName = 'Registered user count'
                const row = tBody.elem('tr')
                ...
                normalizedData.forEach(function (dataValue) {
                  row.elem('td').text(options.valueTransform(dataValue))
                })
              }
            }
          ...
          // Update invisible table in DOM and update table element with newly created table
            containerElement.appendChild(new window.DOMParser().parseFromString(element.toString(), 'text/html').getElementById(elementId))
          })
        }
      }
    }
